...
/*!
    ...
    v1.1 - Changed readPassiveTargetID() to handle multiple UID sizes
         ...
         - Added the following Mifare Classic functions:
             ...
             uint8_t Cerberus_Authenticate (uint8_t * uid, uint8_t uidLen, uint32_t blockNumber, uint8_t keyNumber, uint8_t * keyData)
             uint8_t Cerberus_Read (uint8_t blockNumber, uint8_t * data)
             uint8_t Cerberus_Write (uint8_t blockNumber, uint8_t * data)
...
bool Adafruit_PN532::sendCommandCheckAck(uint8_t *cmd, uint8_t cmdlen, uint16_t timeout) {
  uint16_t timer = 0;
  //Serial.println("  At the command check ack ");
  // write the command
  writecommand(cmd, cmdlen);

  // Wait for chip to say its ready!
  if (!waitready(timeout)) {
	 // Serial.println("  about to return false ");
    return false;
  }
  ...
  if (!readack()) {
    ...
    #endif
	  //Serial.println("  about to return false in the read ");
    return false;
  }
 ...
 // Serial.println("  about to return true ");
...
bool Adafruit_PN532::readPassiveTargetID(uint8_t cardbaudrate, uint8_t * uid, uint8_t * uidLength, uint16_t timeout) {
  ...
  //Serial.println("  at passive target id ");
 ...
 // Serial.println("  in middle at passive target id ");
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532::Cerberus_Authenticate (uint8_t * uid, uint8_t uidLen, uint32_t blockNumber, uint8_t keyNumber, uint8_t * keyData)
{
  uint8_t len;
  uint8_t i;
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532::Cerberus_Read (uint8_t blockNumber, uint8_t * data)
{
  #ifdef MIFAREDEBUG
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532::Cerberus_Write (uint8_t blockNumber, uint8_t * data)
{
  #ifdef MIFAREDEBUG
...
uint8_t Adafruit_PN532::mifareclassic_FormatNDEF (void)
{
  ...
  if (!(Cerberus_Write (1, sectorbuffer1)))
    return 0;
  if (!(Cerberus_Write (2, sectorbuffer2)))
    return 0;
  // Write key A and access rights card
  if (!(Cerberus_Write (3, sectorbuffer3)))
    return 0;
...
uint8_t Adafruit_PN532::mifareclassic_WriteNDEFURI (uint8_t sectorNumber, uint8_t uriIdentifier, const char * url)
{
  ...
  if (!(Cerberus_Write (sectorNumber*4, sectorbuffer1)))
    return 0;
  if (!(Cerberus_Write ((sectorNumber*4)+1, sectorbuffer2)))
    return 0;
  if (!(Cerberus_Write ((sectorNumber*4)+2, sectorbuffer3)))
    return 0;
  if (!(Cerberus_Write ((sectorNumber*4)+3, sectorbuffer4)))
    return 0;
...
bool Adafruit_PN532::waitready(uint16_t timeout) {
  ...
  while(!isready()) {
    if (timeout != 0) {
		//Serial.println("  timer increment ");
      timer += 10;
      if (timer > timeout) {
 ...
 // Serial.println("  at ready return ");
  return true;
}
